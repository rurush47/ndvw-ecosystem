...
public class WolfController : AnimalController<PredatorStates> 
{
	// NEW //////////////////
	...
	public Transform GetMatingPartner(List<Transform> list)
	{
		...
		var target = list.FirstOrDefault(t =>
		{
			...
			var component = t.GetComponent<WolfController>();
			return 
				component != null &&
	...
	void DoAction_Enter()
	{
		...
		switch (currentUrge)
		{
			case Urge.Thirst:
				...
				break;
			case Urge.Hunger:
				gotoTarget.GetComponent<RabbitController>().Die();
	...
	private void spawnChildAnimal()
	{
		...
		var father = gotoTarget.GetComponent<WolfController>();
		...
		var childController = child.GetComponent<WolfController>();
